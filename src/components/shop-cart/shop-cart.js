...
import { cartItemsSelector, cartTotalSelector } from "../../redux/shop/reducer";
...
import { removeFromCart } from "../../redux/shop/actions";
...
export class ShopCart extends connect(store)(LitElement) {
  ...
  _render({ items, total }) {
    return html`
            ...
            <!-- Contents -->
            <p hidden="${items &&
              items.length !== 0}">Please add some products to cart.</p>
            ${items &&
              items.map(
                item => html`
                <div>
